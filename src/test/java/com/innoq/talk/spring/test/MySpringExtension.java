...
public class MySpringExtension implements BeforeAllCallback, TestInstancePostProcessor, AfterAllCallback {
    ...
    static class TestContextManager {
        ...
        private final List<TestExecutionListener> listeners =
                List.of(new DependencyInjectionListener(), new TransactionalListener());
        ...
        void beforeTestClass() {
            listeners.forEach(l -> l.beforeTestClass(ctx));
        }

        void afterTestClass() {
            listeners.reversed().forEach(l -> l.afterTestClass(ctx));
        }

        void prepareTestInstance(Object testInstance) {
            ctx.setTestInstance(testInstance);
            listeners.forEach(l -> l.prepareTestInstance(ctx));
        }
    }
        ...
        public AnnotationConfigApplicationContext getApplicationContext() {
            return CACHE.computeIfAbsent(this.hashCode(), hash -> {
                ...
                if (testProperty != null) {
                    ctx.getEnvironment().getPropertySources().addFirst(new MockPropertySource()
                            .withProperty(testProperty.key(), testProperty.value()));
                }
    ...
    static class DependencyInjectionListener implements TestExecutionListener {
        @Override
        public void prepareTestInstance(TestContext ctx) {
            ctx.getApplicationContext()
                    .getAutowireCapableBeanFactory()
                    .autowireBeanProperties(ctx.getTestInstance(), AUTOWIRE_NO, false);
        }
    }

    static class TransactionalListener implements TestExecutionListener {
        @Override
        public void beforeTestClass(TestContext ctx) {
            System.out.println("TransactionalListener.beforeTestClass");
        }

        @Override
        public void afterTestClass(TestContext ctx) {
            System.out.println("TransactionalListener.afterTestClass");
        }
    }
}
